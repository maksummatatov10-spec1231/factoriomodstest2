...
| `iron-gear-cell` | **AM3 (зелёный сборщик) → железные шестерни**: сундук-запрос (75 плит) → инсертер → AM3 (iron-gear-wheel) → инсертер → пассивный сундук снабжения. JSON + строка + `.md` с координатами/направлениями |
